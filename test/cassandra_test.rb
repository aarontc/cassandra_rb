...
class CassandraTest < Test::Unit::TestCase
  ...
  def setup
    @twitter = CassandraRB.new('Twitter', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @twitter.clear_keyspace!

    @blogs = CassandraRB.new('Multiblog', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @blogs.clear_keyspace!

    @blogs_long = CassandraRB.new('MultiblogLong', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @blogs_long.clear_keyspace!

    @type_conversions = CassandraRB.new('TypeConversions', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @type_conversions.clear_keyspace!

    CassandraRB::WRITE_DEFAULTS[:consistency] = CassandraRB::Consistency::ONE
    CassandraRB::READ_DEFAULTS[:consistency]  = CassandraRB::Consistency::ONE

    @uuids = (0..6).map {|i| SimpleUUID::UUID.new(Time.at(2**(24+i))) }
    @longs = (0..6).map {|i| Long.new(Time.at(2**(24+i))) }
    @composites = [
      CassandraRB::Composite.new([5].pack('N'), "zebra"),
      CassandraRB::Composite.new([5].pack('N'), "aardvark"),
      CassandraRB::Composite.new([1].pack('N'), "elephant"),
      CassandraRB::Composite.new([10].pack('N'), "kangaroo"),
    ]
    @dynamic_composites = [
      CassandraRB::DynamicComposite.new(['i', [5].pack('N')], ['UTF8Type', "zebra"]),
      CassandraRB::DynamicComposite.new(['i', [5].pack('N')], ['UTF8Type', "aardvark"]),
      CassandraRB::DynamicComposite.new(['IntegerType', [1].pack('N')], ['s', "elephant"]),
      CassandraRB::DynamicComposite.new(['IntegerType', [10].pack('N')], ['s', "kangaroo"]),
    ]
  end
  ...
  def test_setting_default_consistency
    assert_nothing_raised do
      @twitter.default_read_consistency = CassandraRB::Consistency::ALL
    end
    assert_equal(CassandraRB::READ_DEFAULTS[:consistency], CassandraRB::Consistency::ALL)

    assert_nothing_raised do
      @twitter.default_write_consistency = CassandraRB::Consistency::ALL
    end
    assert_equal(CassandraRB::WRITE_DEFAULTS[:consistency], CassandraRB::Consistency::ALL)
  end
...
#  end
  ...
  def test_get_range_reversed
    ...
    hash = CassandraRB::OrderedHash[data]
    reversed_hash = CassandraRB::OrderedHash[data.reverse]
  ...
  def test_disconnect_when_not_connected!
    assert_nothing_raised do
      @twitter = CassandraRB.new('Twitter', "127.0.0.1:9160", :retries => 2, :exception_classes => [])
      @twitter.disconnect!
    end
  end
  ...
  if CASSANDRA_VERSION.to_f >= 0.8
    ...
    def test_composite_column_type_conversion
      columns = {}
      @composites.push(
        CassandraRB::Composite.new_from_parts([[20].pack('N'), "meerkat"])
      )
      ...
      columns_in_order = [
        ...
        CassandraRB::Composite.new([5].pack('N'), "zebra"),
        CassandraRB::Composite.new([10].pack('N'), "kangaroo"),
        CassandraRB::Composite.new([20].pack('N'), "meerkat"),
      ]
      ...
      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => CassandraRB::Composite.new([1].pack('N')),
        :finish => CassandraRB::Composite.new([20].pack('N'))
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => CassandraRB::Composite.new([5].pack('N')),
        :finish => CassandraRB::Composite.new([5].pack('N'), :slice => :after)
      ).keys
      assert_equal(columns_in_order[1..2], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => CassandraRB::Composite.new([5].pack('N'), :slice => :after).to_s
      ).keys
      assert_equal(columns_in_order[-2..-1], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :finish => CassandraRB::Composite.new([10].pack('N'), :slice => :before).to_s
      ).keys
    ...
    def test_dynamic_composite_column_type_conversion
      ...
      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => CassandraRB::DynamicComposite.new(['i', [1].pack('N')]),
        :finish => CassandraRB::DynamicComposite.new(['i', [10].pack('N')])
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => CassandraRB::DynamicComposite.new(['IntegerType', [5].pack('N')]),
        :finish => CassandraRB::DynamicComposite.new(['IntegerType', [5].pack('N')], :slice => :after)
      ).keys
      assert_equal(columns_in_order[1..2], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => CassandraRB::DynamicComposite.new(['i', [5].pack('N')], :slice => :after).to_s
      ).keys
      assert_equal([columns_in_order[-1]], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :finish => CassandraRB::DynamicComposite.new(['i', [10].pack('N')], :slice => :before).to_s
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)
  ...
  def test_keyspace_operations
    system = CassandraRB.new 'system'
    keyspace_name = 'robots'
    keyspace_definition = CassandraRB::Keyspace.new :name => keyspace_name,
    ...
    system.add_keyspace keyspace_definition
    CassandraRB.new(keyspace_name).drop_keyspace
    assert system.keyspaces.none? {|it| it == keyspace_name }
  end
